...
function getMapping (cleanTree, inPrefix) {
  ...
  for (field in cleanTree) {
    ...
    // If indexing a mixed, and no es_type specified, default to object
    if ((value.type === 'mixed' && value.es_type === undefined) || (typeof value === 'object' && !value.type)) {
      let childContainEsIndexed = false
      for (prop in value) {
        if (prop.indexOf('es_indexed') && value[prop].es_indexed) {
          childContainEsIndexed = true
          continue
        }
      }

      if (!value.es_indexed && !childContainEsIndexed) {
        implicitFields.push(field)
      }
